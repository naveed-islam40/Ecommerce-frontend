...
export const forgotPassword = (email) =>
  async function (dispatch) {
    try {
      ...
      const config = { headers: { "Content-Type": "application/json" } };
      const { data } = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/password/reset`, email, config);
...
export const resetPassword = (token, passwords) =>
  async function (dispatch) {
    try {
      ...
      const config = { headers: { "Content-Type": "application/json" } };
      const { data } = await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}/api/password/reset/${token}`,
        passwords,
        config
      );
...
export const fetchSingleUser = (userId) =>
  async function (dispatch) {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/admin/user/${userId}`);
